...
public class FileTransferClient {
    public static void main(String[] args)
        ...
        FileTransferImpl stub = (FileTransferImpl) registry.lookup("FileTransfer");
        Registry registry2 = LocateRegistry.getRegistry("localhost", 1100);
        FileTransferImpl stub2 = (FileTransferImpl) registry.lookup("FileTransfer");
        ...
        do {
            int random_client = (int) Math.floor(Math.random() *5 + 0); //0 a 4

            // do {
            // decision_sends = false;
            suma = 0;
            int random_int = (int) Math.floor(Math.random() * (11 - 1 + 1) + 1);
            ...
            if (decision_sends == true) {
                ...
                try (BufferedInputStream bis = new BufferedInputStream(new FileInputStream(file))) {
                    ...
                    while ((bytesRead = bis.read(buffer)) != -1) {
                        stub.transferFile(buffer, file.getName(), random_int, (int)chunkSize);
                    }
                }
            }
